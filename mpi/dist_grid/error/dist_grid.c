...
int main(int argc, char **argv) {
    graph_node*** initial_global_graph, ***final_global_graph; /**< Initial and final graph */
    graph_node*** local_graph; /**< Local Graph representation - partition of the total graph */

    graph_node* it;
    int nprocs = 1, rank = ROOT; //Number of processes and rank of the process
    int size = 0;
    int i = 0, j = 0;
    int count;
    int g;
...
/*************************************************************RAN BY ROOT *************************************************************************************/
    if(rank == ROOT){
        ...
        //Allocate space for the sendcouut to each rank
        sendcounts 	= (int *) calloc(sizeof(int),nprocs);
        //Read the number of alive nodes and create their sendcount value
        ...
        for(i = 0; i < nprocs; i++){
            debug_print("Root computing number of nodes proc %d: %d\n", nprocs, sendcounts[i]);
            debug_print("Root computing displacements: %d\n", displs[i]);
        ...
        //Create the initial global graph from the file
        initial_global_graph = initGraph(size);
        fclose(fp);fp = fopen(file, "r"); //Close and reopen file to reset fp
        while(fgets(buffer, BUFFER_SIZE, fp)){
            if(sscanf(buffer, "%d %d %d\n", &x, &y, &z) == 3){
                initial_global_graph[x][y] = graphNodeInsert(initial_global_graph[x][y], z, ALIVE); /* Insert live nodes in the initial graph*/
            }
        }

        sendbuffer = (node*) malloc(alive_nodes*sizeof(node)); //Allocate an array with all the alive nodes
        ...
        debug_print("Counted %d\n", count);

        receivebuffer = (node *) calloc(cells_receive, sizeof(node));  /* Convert the coordinates read into an array for sending */
        ...
        debug_print("Created local graph...\n");
        for(int i=0; i<cells_receive; i++){
        ...
        //for(g=0; g<generations; g++){  //Calculate neighbours for all (excluding in our side frontiers - can't add wrap arounds)
            ...
            //Send our frontiers
            int low_rank = (rank-1) < 0 ? (nprocs-1) : (rank-1); //To which ranks do we send?
            int high_rank = (rank+1) >= nprocs ? (0) : (rank+1); //To which ranks do we send?
            ...
            rank_print(rank); debug_print("HAS ALL THE FRONTIERS\n");

            //We have all the frontiers here, go over our side of the high frontier and take in consideration all those that are on the high frontier that was sent to us
            /*for(y=0; y < size; y++){
                for(it=local_graph[(BLOCK_SIZE(rank, nprocs, size) - 1)][y]; it!=NULL; it=it->next){
                    if(it->state == ALIVE){
